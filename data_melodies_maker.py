from pydub import AudioSegment  # Import AudioSegment class for audio processing
from pydub.generators import Sine  # Import Sine generator for creating sine wave audio
import simpleaudio as sa  # Import simpleaudio for playing audio
import io  # Import io for working with file-like objects
...
# Define lists of musical notes for different scales
g_major_notes = [
...
# List to store notes from file
file_notes = []

# Beats per minute and beat duration in milliseconds
bpm = 120
beat_duration_ms = int((60 / bpm) * 1000)  # Duration of each beat in milliseconds

# Function to get note name from ASCII character
...
# Function to read bytes from file and convert to musical notes
...
# Function to play a tone with a specified frequency        
...
# Print the musical key based on the file extension
...
# Read bytes from the file and convert to musical notes
read_file_bytes(file_path)

# Initialize the previous_note variable with the first note in the list of file_notes
previous_note = file_notes[0]

# Initialize a counter to track the number of consecutive repeated notes
num_repeated_notes = 1

# Loop through the list of file_notes starting from the second note
for i in range (1, len(file_notes) - 1):
    # Assign the current note being processed to the variable current_note
    current_note = file_notes[i]
    
    # Check if the note is within the current musical key
    if current_note not in current_key: 
        # Print a message indicating that the script is waiting
        print("sleeping if")
        # Sleep for half of the beat duration to create a pause
    ...
    # Check if the current note is the same as the previous note
        ...
        # Increment the counter to keep track of consecutive repeated notes
        print("incrementing num_repeated_notes")
        num_repeated_notes += 1
    else:
        # If the current note is different from the previous note, play the note
        print("playing the note!")
        print("num_repeated_notes:")
        print(num_repeated_notes)

        # Play the note with the appropriate frequency and duration
        play_frequency(note_frequencies[current_note], num_repeated_notes * beat_duration_ms)
        # Reset the counter for consecutive repeated notes to 1
        num_repeated_notes = 1
        
    # Messages to show the current and previous notes being processed
    print("previous_note:")
    print(previous_note)
    print("current_note:")
    print(current_note)

    # Update the previous_note variable for the next iteration of the loop
